...
def process_image_getColor(imageString):
	image_color_quantities = {}
	image=imageString#Image.open(io.BytesIO(imageString))
	...
	for color, strength in final_colors.items():
		if x==0:
			...
			objectColor=color.__name__
			colorAccuracy=str(strength)
			x=1

	return objectColor,colorAccuracy;
	#image.show()

if __name__ == '__main__':
	image = Image.open("./data/actor/dev/img/9M.jpg")

#imagePath =Image.open("./data/actor/dev/img/9M.jpg")
#A,B=process_image_getColor(imagePath)
